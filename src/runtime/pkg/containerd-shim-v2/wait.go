...
func wait(ctx context.Context, s *service, c *container, execID string) (int32, error) {
	...
	if execID == "" {
		...
		publishExitStatus := func() {
			// Publish TaskExit while holding s.mu, before exposing the Wait
			// response. This prevents a concurrent Delete RPC from publishing
			// TaskDelete first.
			s.mu.Lock()
		...
		if c.cType.IsSandbox() {
			...
			s.teardownOnce.Do(func() {
				...
				// Do not expose the stopped task until Stop has restored assigned
				// devices. Containerd may start task deletion as soon as either the
				// Wait response or TaskExit becomes visible.
				publishExitStatus()
				published = true
